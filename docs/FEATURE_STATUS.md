...
| 🔍 Filtri avanzati | 🟠 | Genere/prezzo/OS fatti (OS ora affidabile: colonne windows/mac/linux ribackfillate il 2026-07-09, prima erano tutte `false` per un volume dati stantio); VR fatto (categorie Steam "VR Support"/"VR Only"/ecc., checkbox in `CatalogPage.tsx`); voto minimo utenti fatto (media `reviews.rating`, distinta dal rating Steam del dataset — subquery correlata in `GameRepository.withFilters`, select in `CatalogPage.tsx`); manca ancora colore copertina (script esistente ma non integrato, richiederebbe elaborare le immagini di 122k giochi, vedi `docs/COVER_COLOR_FILTER_PLAN.md`) — issue #11 |
...
---
*Ultimo aggiornamento: 2026-07-13 — issue #29: scelta la libreria di animazioni JS, `@formkit/auto-animate`. Preferita a framer-motion/react-spring: single hook `useAutoAnimate()` senza configurazione per proprietà, coerente con le animazioni esistenti (tutte CSS/Tailwind semplici) e col criterio "semplicità sopra flessibilità" dato per questa scelta. Applicata ai punti dove liste già renderizzate cambiano a runtime (non allo stagger di comparsa iniziale, già coperto da `animate-fade-in-up`): griglia di `LibraryPage` e le due liste di `FriendsPage` (richieste in sospeso, amici):*

*Precedente (2026-07-10) — chiusa #27 (sign-off FK di `db/init.sql`): ogni FK verificata contro i `@JoinColumn` JPA corrispondenti (games→developers/publishers, game_genres, game_categories, backlog→users/games, friends, reviews, game_sessions) — tutte corrette, nessun bug. Trovate però 3 tabelle in `init.sql` senza alcun codice che le usa: `wishlist` (la wishlist reale vive come `backlog.status = 'wishlist'`, non su questa tabella), `game_developers` (Game ha già developer_id diretto su `games`, mai popolata da `populate_db.py`), `user_settings` (mai letta/scritta dal backend). Non rimosse in questo giro — verifica, non nuovo lavoro di schema — ma segnalate come dead schema per una futura pulizia:*

*Ultimo aggiornamento: 2026-07-13 — issue #11: aggiunto il filtro "voto minimo utenti" nel catalogo (`GameFilterRequest.minUserRating`, subquery correlata `AVG(reviews.rating)` in `GameRepository.withFilters` — stesso pattern della subquery di esclusione contenuti per adulti, ma con AVG invece di EXISTS; un gioco senza recensioni ha AVG NULL e quindi non passa il filtro, comportamento corretto). Corretta anche la riga "Filtri avanzati" qui sotto, che segnalava ancora VR come da collegare: verificato nel codice che è già completo (categorie Steam + checkbox in `CatalogPage.tsx`), la voce era rimasta stantia. Resta solo colore copertina, issue a parte:*
*Precedente (stesso giorno) — chiusa #27 (sign-off FK di `db/init.sql`): ogni FK verificata contro i `@JoinColumn` JPA corrispondenti (games→developers/publishers, game_genres, game_categories, backlog→users/games, friends, reviews, game_sessions) — tutte corrette, nessun bug. Trovate però 3 tabelle in `init.sql` senza alcun codice che le usa: `wishlist` (la wishlist reale vive come `backlog.status = 'wishlist'`, non su questa tabella), `game_developers` (Game ha già developer_id diretto su `games`, mai popolata da `populate_db.py`), `user_settings` (mai letta/scritta dal backend). Non rimosse in questo giro — verifica, non nuovo lavoro di schema — ma segnalate come dead schema per una futura pulizia:*

*Precedente (2026-07-10) — chiusa #182: `HeroSection.tsx` ora sceglie a caso uno tra 3 video gameplay in whitelist (ELDEN RING, Baldur's Gate 3, Grand Theft Auto V — tutti verificati embeddabili via oEmbed YouTube, nessun trailer con loghi/testo), invece del singolo video hardcoded (che tra l'altro non era nemmeno di un gioco del catalogo). Scelta persistita in `sessionStorage` così non cambia a ogni render/tornando sulla home nella stessa sessione. Fallback su embed fallito (PR #165) generalizzato per funzionare con qualunque video della whitelist, non solo quello hardcoded prima. Curare/ampliare la whitelist resta una decisione di contenuto, fuori scope qui:*
