    ...
    <div className="relative">
      ...
      <section className="max-w-5xl mx-auto px-4 py-12">
        <div className="grid sm:grid-cols-3 gap-5">
          {[
            { Icon: Gamepad2, titleKey: "home.features.catalog.title", descKey: "home.features.catalog.desc", to: "/catalog" },
            { Icon: Heart, titleKey: "home.features.library.title", descKey: "home.features.library.desc", to: isAuthenticated ? "/library" : "/login" },
            { Icon: BarChart3, titleKey: "home.features.stats.title", descKey: "home.features.stats.desc", to: isAuthenticated ? "/profile" : "/login" },
          ].map(({ Icon, titleKey, descKey, to }) => (
            <Link
              to={to}
              key={titleKey}
      ...
      <section className="max-w-5xl mx-auto px-4 pb-12">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatCard label={t("stats.inProgress")} value={userStats?.playingCount ?? 0} color="text-blue-400" />
          <StatCard label={t("stats.finished")} value={userStats?.finishedCount ?? 0} color="text-emerald-400" />
          <StatCard label={t("stats.abandoned")} value={userStats?.abandonedCount ?? 0} color="text-vz-pink" />
        </div>
      </section>
